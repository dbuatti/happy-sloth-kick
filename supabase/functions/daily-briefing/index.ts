  ...
  try {
    ...
    const [tasksRes, appointmentsRes, weeklyFocusRes, sleepRecordRes] = await Promise.all([
      supabaseAdmin.from('tasks')
        .select('description, status, priority, due_date, section_id, recurring_type, original_task_id')
        .eq('user_id', userId)
    ...
    const sleepRecord = sleepRecordRes.data;

    // Process tasks for AI prompt
    const pendingTasks = tasks.filter(t => t.status === 'to-do' && (t.due_date === todayDateString || !t.due_date));
    const completedTasks = tasks.filter(t => t.status === 'completed' && new Date(t.updated_at).toISOString().split('T')[0] === todayDateString);
    const overdueTasks = tasks.filter(t => t.status === 'to-do' && t.due_date && new Date(t.due_date) < new Date(todayDateString));
    ...
    - Pending tasks for today: ${JSON.stringify(pendingTasks.map(t => ({ description: t.description, priority: t.priority, due_date: t.due_date })))}
    - Completed tasks today: ${JSON.stringify(completedTasks.map(t => t.description))}
    - Overdue tasks: ${JSON.stringify(overdueTasks.map(t => ({ description: t.description, due_date: t.due_date })))}
    - Appointments today: ${JSON.stringify(appointments.map(a => ({ title: a.title, start_time: a.start_time, end_time: a.end_time })))}
