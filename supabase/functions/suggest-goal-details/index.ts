...
serve(async (req: Request) => { // Explicitly type req as Request
  ...
  try {
    ...
    const { text, categories, currentDate } = requestBody; // Correctly destructure 'text'

    if (!text) { // Check for 'text' instead of 'prompt'
    ...
    const responseText = response.text(); // Changed variable name from 'text' to 'responseText' to avoid conflict
