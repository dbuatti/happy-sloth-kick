...
import { format, parseISO, isValid, setHours, setMinutes, getHours, getMinutes } from 'date-fns';
import { QueryClient } from '@tanstack/react-query';
import { showError, showSuccess } from '@/utils/toast';
import { addReminder, dismissReminder } from '@/context/ReminderContext'; // Assuming these are available
...
interface MutationContext {
  ...
  processedTasks: Task[]; // Pass processedTasks for virtual task handling
}
...
export const archiveAllCompletedTasksMutation = async (
  ...
  await bulkUpdateTasks( { status: 'archived' }, completedTaskIds, { userId, processedTasks, queryClient: {} as QueryClient, inFlightUpdatesRef: {} as React.MutableRefObject<Set<string>>, categoriesMap: {} as Map<string, string>, invalidateTasksQueries: () => {}, invalidateSectionsQueries: () => {}, invalidateCategoriesQueries: () => {} });
};
...
export const markAllTasksInSectionCompletedMutation = async (
  ...
  await bulkUpdateTasks({ status: 'completed' }, tasksToComplete, { userId, processedTasks, queryClient: {} as QueryClient, inFlightUpdatesRef: {} as React.MutableRefObject<Set<string>>, categoriesMap: {} as Map<string, string>, invalidateTasksQueries: () => {}, invalidateSectionsQueries: () => {}, invalidateCategoriesQueries: () => {} });
};
