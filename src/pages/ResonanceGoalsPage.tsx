...
const ResonanceGoalsPage: React.FC<ResonanceGoalsPageProps> = ({ isDemo = false, demoUserId }) => {
  ...
  const goalsByTimeframe = useMemo(() => {
    ...
    goals.forEach(goal => {
      // Group ALL goals by type, without filtering by parent_goal_id here
    ...
    <div className="flex-1 overflow-auto p-4 lg:p-6">
      ...
      <div className="grid gap-6">
        {goalTypeOrder.map(type => {
          // Pass the complete list of goals to the section, so it can manage parent/child relationships
          return (
            <ResonanceGoalTimelineSection
              key={type}
              goalType={type}
              allGoals={goals} {/* Pass ALL goals from the page */}
              goals={goalsByTimeframe.get(type) || []} {/* This will now contain all goals of that type, including sub-goals */}
