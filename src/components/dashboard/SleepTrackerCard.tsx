...
const SleepTrackerCard: React.FC = () => {
  ...
  useEffect(() => {
    if (editingRecord) {
      setDate(parseISO(editingRecord.date));
      setBedTime(editingRecord.bed_time ? parseISO(`2000-01-01T${editingRecord.bed_time}`) : undefined);
      setLightsOffTime(editingRecord.lights_off_time ? parseISO(`2000-01-01T${editingRecord.lights_off_time}`) : undefined);
      setWakeUpTime(editingRecord.wake_up_time ? parseISO(`2000-01-01T${editingRecord.wake_up_time}`) : undefined);
      setGetOutOfBedTime(editingRecord.get_out_of_bed_time ? parseISO(`2000-01-01T${editingRecord.get_out_of_bed_time}`) : undefined);
      setTimeToFallAsleepMinutes(editingRecord.time_to_fall_asleep_minutes || undefined);
      setSleepInterruptionsCount(editingRecord.sleep_interruptions_count || undefined);
      setSleepInterruptionsDurationMinutes(editingRecord.sleep_interruptions_duration_minutes || undefined);
      setTimesLeftBedCount(editingRecord.times_left_bed_count || undefined);
      setPlannedWakeUpTime(editingRecord.planned_wake_up_time ? parseISO(`2000-01-01T${editingRecord.planned_wake_up_time}`) : undefined);
    } else {
      resetForm();
    }
  }, [editingRecord]);
  ...
  if (error) {
    return (
      <Card>
        ...
        <CardContent className="text-red-500">Error loading sleep records.</CardContent>
      </Card>
    );
  }
            ...
            <div key={record.id} className="flex items-center justify-between p-2 border rounded-md text-sm">
              <div className="flex-grow">
                ...
                <p className="text-xs text-gray-500">
                  {record.bed_time ? format(parseISO(`2000-01-01T${record.bed_time}`), 'h:mm a') : 'N/A'} -{' '}
                  {record.wake_up_time ? format(parseISO(`2000-01-01T${record.wake_up_time}`), 'h:mm a') : 'N/A'}
                </p>
              </div>
