              ...
              <CommandList>
                ...
                <CommandGroup heading="Actions">
                  <CommandItem onSelect={() => handleSelect(() => setIsAddTaskDialogOpen(true))}>
                    <Plus className="mr-2 h-3.5 w-3.5" /> {/* Changed h-4 w-4 to h-3.5 w-3.5 */}
                    <span>Add New Task</span>
                  </CommandItem>
                  <CommandItem onSelect={() => handleSelect(() => navigate('/'))}>
                    <Home className="mr-2 h-3.5 w-3.5" /> {/* Changed h-4 w-4 to h-3.5 w-3.5 */}
                    <span>Go to Daily Tasks</span>
                  </CommandItem>
                  <CommandItem onSelect={() => handleSelect(() => setCurrentDate(new Date()))}>
                    <CalendarDays className="mr-2 h-3.5 w-3.5" /> {/* Changed h-4 w-4 to h-3.5 w-3.5 */}
                    <span>Go to Today</span>
                  </CommandItem>
                  <CommandItem onSelect={() => handleSelect(() => navigate('/focus'))}>
                    <Target className="mr-2 h-3.5 w-3.5" /> {/* Changed h-4 w-4 to h-3.5 w-3.5 */}
                    <span>Go to Focus Mode</span>
                  </CommandItem>
                  <CommandItem onSelect={() => handleSelect(() => navigate('/projects'))}>
                    <LayoutGrid className="mr-2 h-3.5 w-3.5" /> {/* Changed h-4 w-4 to h-3.5 w-3.5 */}
                    <span>Go to Project Balance</span>
                  </CommandItem>
                  <CommandItem onSelect={() => handleSelect(() => navigate('/schedule'))}>
                    <CalendarClock className="mr-2 h-3.5 w-3.5" /> {/* Changed h-4 w-4 to h-3.5 w-3.5 */}
                    <span>Go to Time Blocks</span>
                  </CommandItem>
                  <CommandItem onSelect={() => handleSelect(() => navigate('/my-hub'))}>
                    <BarChart3 className="mr-2 h-3.5 w-3.5" /> {/* Changed h-4 w-4 to h-3.5 w-3.5 */}
                    <span>Go to My Hub</span>
                  </CommandItem>
                  <CommandItem onSelect={() => handleSelect(() => navigate('/settings'))}>
                    <Settings className="mr-2 h-3.5 w-3.5" /> {/* Changed h-4 w-4 to h-3.5 w-3.5 */}
                    <span>Go to Settings</span>
                  </CommandItem>
                  <CommandItem onSelect={() => handleSelect(() => setCurrentDate(prev => new Date(prev.setDate(prev.getDate() - 1))))}>
                    <ChevronLeft className="mr-2 h-3.5 w-3.5" /> {/* Changed h-4 w-4 to h-3.5 w-3.5 */}
                    <span>Previous Day</span>
                  </CommandItem>
                  <CommandItem onSelect={() => handleSelect(() => setCurrentDate(prev => new Date(prev.setDate(prev.getDate() + 1))))}>
                    <ChevronRight className="mr-2 h-3.5 w-3.5" /> {/* Changed h-4 w-4 to h-3.5 w-3.5 */}
                    <span>Next Day</span>
                  </CommandItem>
                </CommandGroup>
                ...
                {user && (
                  <CommandGroup heading="Account">
                    <CommandItem onSelect={() => handleSelect(handleSignOut)}>
                      <LogOut className="mr-2 h-3.5 w-3.5" /> {/* Changed h-4 w-4 to h-3.5 w-3.5 */}
                      <span>Sign Out</span>
                    </CommandItem>
                  </CommandGroup>
                )}
                  ...
                  <CommandGroup heading="Sections">
                    {sections.map(section => (
                      <CommandItem key={section.id} onSelect={() => handleSelect(() => {
                        ...
                        <FolderOpen className="mr-2 h-3.5 w-3.5" /> {/* Changed h-4 w-4 to h-3.5 w-3.5 */}
