...
const HabitHistoryGrid: React.FC<HabitHistoryGridProps> = ({
  ...
  const monthLabels = useMemo(() => {
    ...
    gridDays.forEach((dayData, index) => {
      const month = format(dayData.date, 'MMM');
      ...
      if (index === gridDays.length - 1 && currentMonth !== null) {
        labels.push({ month: currentMonth, span: currentMonthSpan });
      }
    });
    return labels;
      ...
      <div className="grid gap-0.5 mb-1" style={{ gridTemplateColumns: `repeat(${weeksToShow}, 1fr)` }}>
        {monthLabels.map((label, index) => (
          <div 
            ...
            style={{ gridColumn: `span ${Math.ceil(label.span / 7)}` }} // Span across weeks
          >
            {label.month}
          </div>
        ))}
