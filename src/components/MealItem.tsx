...
const MealItem: React.FC<MealItemProps> = ({ meal, onUpdate, isDemo = false, isPlaceholder }) => {
  ...
  const getMealIcon = (type: MealType) => {
    switch (type) {
      case 'breakfast': return <Coffee className="h-3 w-3 text-muted-foreground" />; // Further reduced icon size
      case 'lunch': return <Soup className="h-3 w-3 text-muted-foreground" />; // Further reduced icon size
      case 'dinner': return <UtensilsCrossed className="h-3 w-3 text-muted-foreground" />; // Further reduced icon size
    }
  };

  const getIngredientStatusIcon = () => {
    if (hasIngredients) {
      return <CheckCircle2 className="h-3 w-3 text-green-500" />; // Further reduced icon size
    } else {
      return <ShoppingCart className="h-3 w-3 text-red-500" />; // Further reduced icon size
    }
  };
  ...
  return (
    <div
      className={cn(
        "relative flex flex-col gap-0 py-0 px-2 rounded-xl shadow-sm transition-all duration-200 ease-in-out", // Main container padding set to py-0
        "border-l-4",
    ...
    >
      <div className="flex items-center justify-between py-0"> {/* Reduced vertical padding to py-0 */}
        <div className="flex items-center gap-0.5"> {/* Reduced gap */}
          {getMealIcon(meal.meal_type)}
          <span className={cn(
            "text-[0.7rem] font-semibold", // Reduced font size
          ...
          {name.trim() !== '' && (
            <UtensilsCrossed className="h-2.5 w-2.5 text-primary/70 ml-0.5" /> // Further reduced icon size
          )}
        </div>
        <div className="flex items-center gap-0.5"> {/* Reduced gap */}
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                variant="ghost"
                size="icon"
                className={cn("h-4 w-4", isDemo && "cursor-not-allowed")} // Reduced button size
          ...
          <Checkbox
            ...
            disabled={isDemo}
            className="h-3 w-3" // Reduced checkbox size
          />
      ...
      <Input
        ...
        className={cn(
          "text-sm font-bold border-none bg-transparent shadow-none focus-visible:ring-0 focus-visible:ring-offset-0 p-0 h-auto leading-tight", // Reduced font size
      ...
      <Textarea
        ...
        rows={1}
        className={cn(
          "text-xs border-none bg-transparent shadow-none focus-visible:ring-0 focus-visible:ring-offset-0 p-0 h-auto resize-none leading-tight", // Reduced font size
          "hover:border-b hover:border-input focus-visible:border-b focus-visible:border-primary",
          isCompleted && "line-through text-muted-foreground",
          isPlaceholder && "placeholder:text-muted-foreground/50"
        )}
