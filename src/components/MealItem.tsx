...
const MealItem: React.FC<MealItemProps> = ({ meal, onUpdate, isDemo = false, isPlaceholder }) => {
  ...
  const getMealIcon = (type: MealType) => {
    switch (type) {
      case 'breakfast': return <Coffee className="h-3.5 w-3.5 text-muted-foreground" />;
      case 'lunch': return <Soup className="h-3.5 w-3.5 text-muted-foreground" />;
      case 'dinner': return <UtensilsCrossed className="h-3.5 w-3.5 text-muted-foreground" />;
    }
  };
  ...
  const getIngredientStatusIcon = () => {
    if (hasIngredients) {
      ...
      return <ShoppingCart className="h-3.5 w-3.5 text-red-500" />;
    }
  };
  ...
  return (
    ...
    >
      ...
      <Input
        ...
        className={cn(
          "text-base font-bold border-none bg-transparent shadow-none focus-visible:ring-0 focus-visible:ring-offset-0 p-0 h-auto leading-tight",
